      ...
      <main className="article-page">
        <div className="container">
          ...
          {(() => {
            ...
            // Matching strict (regex word boundary) pour éviter les faux positifs (ex: "maman" ne déclenche PAS Fête des Mères).
            const now = new Date();
            const within = (start: string, end: string) => now >= new Date(start) && now <= new Date(end);
            if (within("2026-01-20", "2026-02-16") && /saint[- ]valentin|\bvalentine\b|cadeau couple|cadeau romantique/.test(blob)) {
              push({ url: "/saint-valentin-2026", label: "Cadeaux Saint-Valentin", emoji: "💕" });
            }
            if (within("2026-04-20", "2026-06-10") && /f[êe]te[- ]des[- ]m[èe]res|cadeau maman|f[êe]te maman/.test(blob)) {
              push({ url: "/fete-des-meres-2026", label: "Cadeaux Fête des Mères", emoji: "🌸" });
            }
            if (within("2026-06-08", "2026-06-25") && /f[êe]te[- ]des[- ]p[èe]res|cadeau papa|f[êe]te papa/.test(blob)) {
              push({ url: "/fete-des-peres-2026", label: "Cadeaux Fête des Pères", emoji: "👨" });
            }
            if (within("2026-06-15", "2026-07-25") && /soldes d['’]?[ée]t[ée]|soldes[- ]ete/.test(blob)) {
              push({ url: "/soldes-ete-2026", label: "Soldes d'Été", emoji: "☀️" });
            }
            if (within("2026-08-01", "2026-09-15") && /rentr[ée]e[- ]scolaire|fournitures?[- ]scolaires?|\bcartable\b/.test(blob)) {
              push({ url: "/rentree-scolaire-2026", label: "Rentrée Scolaire", emoji: "🎒" });
            }
            if (within("2026-10-01", "2026-11-02") && /\bhalloween\b|d[ée]guisement halloween|deco halloween|\bcitrouille\b/.test(blob)) {
              push({ url: "/halloween-2026", label: "Halloween", emoji: "🎃" });
            }
            if (within("2026-11-10", "2026-12-02") && /black[- ]friday|cyber[- ]monday/.test(blob)) {
              push({ url: "/black-friday-2026", label: "Black Friday", emoji: "🛍️" });
            }
            if (within("2026-09-15", "2026-12-25") && /calendrier[- ]avent|calendrier de l['’]avent/.test(blob)) {
              push({ url: "/calendrier-avent-2026", label: "Calendrier de l'Avent", emoji: "🎄" });
            }
            if (within("2026-11-28", "2026-12-26") && /\bno[ëe]l\b|cadeau no[ëe]l|p[èe]re no[ëe]l/.test(blob)) {
