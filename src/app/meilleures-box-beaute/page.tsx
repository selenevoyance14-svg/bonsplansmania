...
export default function BeautyBoxesHub() {
  ...
  return <><Header activePage="/meilleures-box-beaute" /><main className={styles.main}>
    ...
    <section className={`container ${styles.section}`}><h2>Comparatif rapide des box beauté</h2><div className={styles.tableWrap}><table><thead><tr><th>Box</th><th>Prix constaté</th><th>Contenu</th><th>Formule</th></tr></thead><tbody>{BEAUTY_BOXES_2026.map((box) => <tr key={box.brand}><td><Link href={box.articleHref}><strong>{box.brand}</strong><br />{box.name}</Link></td><td><strong>{box.price}</strong>{box.value ? <small>{box.value}</small> : null}</td><td>{box.contents}</td><td>{box.commitment}</td></tr>)}</tbody></table></div></section>
    <section className={`container ${styles.section}`}><h2>Les box beauté disponibles</h2><BeautyBoxFilters /></section>
