...
export default function BeautyBoxesHub() {
  ...
  const productTests = getAllArticles()
    .filter(({ meta }) => meta.category === "test-gratuit" && !meta.expired)
    .sort((a, b) => (b.meta.updated || b.meta.date).localeCompare(a.meta.updated || a.meta.date))
    .slice(0, 8);
  const itemList = { "@context": "https://schema.org", "@type": "ItemList", name: "Meilleures box beauté 2026", numberOfItems: beautyBoxCatalog.length, itemListElement: beautyBoxCatalog.map((box, index) => ({ "@type": "ListItem", position: index + 1, name: `${box.brand} ${box.name}`, url: `https://bonsplansmania.fr${box.articleHref}` })) };
  return <><Header activePage="/meilleures-box-beaute" /><main className={styles.main}>
    <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(itemList) }} />
    <section className={styles.hero}><div className="container"><p className={styles.eyebrow}>Comparateur et agrégateur beauté</p><h1>Box beauté 2026 : prix, avis et comparatif</h1><p className={styles.intro}>Comparez les box par budget, univers et type de formule, puis retrouvez les dernières campagnes de tests et d’échantillons gratuits publiées sur Bons Plans Mania.</p><div className={styles.stats}><span><strong>{beautyBoxCatalog.length}</strong> offres de box référencées</span><span><strong>{productTests.length}</strong> tests récents à découvrir</span><span><strong>Prix datés</strong> et conditions affichées</span></div></div></section>
    ...
    <section className={`container ${styles.section}`} id="tests-produits"><div className={styles.sectionIntro}><div><p className={styles.sectionKicker}>Échantillons et campagnes</p><h2>Tests de produits et échantillons gratuits</h2></div><Link href="/tests-produits-gratuits-2026">Voir tous les tests produits</Link></div><p className={styles.explainer}>Ces campagnes sont gratuites, mais une inscription ne garantit pas la sélection. Les dates et critères de participation sont précisés dans chaque fiche.</p><div className={styles.testGrid}>{productTests.map(({ meta }) => <article className={styles.testCard} key={meta.slug}><Link href={`/article/${meta.slug}`} className={styles.testImage}><img src={meta.image} alt={meta.imageAlt} loading="lazy" /></Link><div><p className={styles.testMeta}>Test produit · vérifié le {new Intl.DateTimeFormat("fr-FR", { day: "numeric", month: "long", year: "numeric", timeZone: "Europe/Paris" }).format(new Date(`${meta.updated || meta.date}T12:00:00`))}</p><h3><Link href={`/article/${meta.slug}`}>{meta.title}</Link></h3><p>{meta.description}</p><Link className={styles.testLink} href={`/article/${meta.slug}`}>Voir les conditions</Link></div></article>)}</div></section>
