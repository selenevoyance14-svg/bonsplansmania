...
export default function SolarGuidePage() {
  ...
  return <><Header activePage="/guide-solaires-2026" /><main className={styles.main}>
    ...
    <section className={`container ${styles.section}`}><h2>Comparer rapidement les solaires</h2><div className={styles.tableWrap}><table><thead><tr><th>Produit</th><th>Protection</th><th>Usage</th><th>Prix constaté</th></tr></thead><tbody>{SOLAR_GUIDE_2026.map((item) => <tr key={`${item.brand}-${item.name}`}><td><Link href={item.articleHref}><strong>{item.brand}</strong><br />{item.name}</Link></td><td>{item.spf}<small>{item.format}</small></td><td>{item.usages.join(" · ")}</td><td><strong>{item.price}</strong><small>le {item.checkedAt}</small></td></tr>)}</tbody></table></div></section>
    <section className={`container ${styles.section}`}><h2>Trouver un solaire par marque et besoin</h2><SolarFilters /></section>
